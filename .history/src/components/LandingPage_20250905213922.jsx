...
export default function LandingPage() {
    ...
    const recentExpenses = expenses.some(
        (expense) => new Date(expense.date) >= new Date(new Date().setDate(new Date().getDate() - 3))
      );
      const recentExpenses = hasRecent ? expenses.filter((expense) => expense.date >= new Date(new Date().setDate(new Date().getDate() - 3))) : [];
        ...
        <div className='h-full w-full bg-black rounded-xl px-12 pt-16 pb-8'>
            ...
            <div className='w-full rounded-2xl border border-gray-800'>
                    <div className='px-6 py-4 border-b border-gray-800'>
                        <h2 className='text-2xl font-semibold text-white'>Your Account Information</h2>
                    </div>
                    <div className='px-6 py-4'>
                        <div className='flex items-center justify-between'>
                            <h3 className='text-lg font-semibold text-white'>Total Balance</h3>
                            <p className='text-lg font-semibold text-white'>{formatAmount(TotalBalance)}</p>
                        </div>
                    </div>
                    <div className='px-6 py-4'>
                        <div className='flex items-center justify-between'>
                            <h3 className='text-lg font-semibold text-white'>Total Expenses</h3>
                            <p className='text-lg font-semibold text-white'>{formatAmount(TotalExpenses)}</p>
                        </div>
                    </div>
                    <div className='px-6 py-4'>
                        <div className='flex items-center justify-between'>
                            <h3 className='text-lg font-semibold text-white'>Total Income</h3>
                            <p className='text-lg font-semibold text-white'>{formatAmount(TotalIncome)}</p>
                        </div>
                    </div>
                </div>
                <div className='w-full rounded-2xl border border-gray-800'>
                    <div className='px-6 py-4 border-b border-gray-800'>
                        <h2 className='text-2xl font-semibold text-white'>Recent Expenses in past 7 days</h2>
                    </div>
                        ...
                        <div className="relative">
                            <table className="w-full table-fixed">
                                ...
                                <thead className="text-gray-400 text-sm bg-black sticky top-0 z-10">
                                    <tr>
                                        <th className='py-3 px-6 text-left font-bold text-xl'>Title</th>
                                        <th className='py-3 px-6 text-left font-bold text-xl'>Category</th>
                                        <th className='py-3 px-6 text-left font-bold text-xl'>Date</th>
                                        <th className='py-3 px-6 text-right font-bold text-xl'>Amount</th>
                                    </tr>
                                </thead>
                            </table>
