                  ...
                  <tbody className='divide-y divide-gray-800 text-gray-200'>
                    {expenses && expenses.length > 0 ? (
                      expenses.map((expense) => (
                        <tr key={expense.id}>
                          <td className='py-4 px-6 text-left'>{expense.title || '—'}</td>
                          <td className='py-4 px-6 text-left'>
                            <span className={`inline-flex items-center rounded-full text-left px-3 py-1 text-sm font-medium ${categoryBadgeClasses(expense.category)}`}>
