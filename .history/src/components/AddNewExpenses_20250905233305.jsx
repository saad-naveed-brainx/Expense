...
export default function Expenses() {
  ...
  return (
    <div className='bg-black rounded-xl h-full flex flex-col py-8 px-12 text-white gap-5 overflow-y-auto'>
          ...
          <form id='expenseForm' className='p-10 flex flex-col gap-8' onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className='flex gap-4 items-center'>
              ...
              <div className='flex items-center gap-2 flex-1'>
                <input type="checkbox" name="reimbersable" id="reimbersable" className='w-5 h-5 rounded-xl ' {...register("reimbersable")} defaultValue={id ? expense.reimbersable : false} />
            ...
            <div className='flex gap-4 items-start'>
              ...
              <input
                ...
                defaultValue={id ? expense.amount : ""}
                />
                {errors.amount && <p className="text-red-500">{errors.amount.message}</p>}
              </div>
              <div className='w-full flex flex-col gap-2'>
                <input type="date" placeholder='Select Date' defaultValue={id ? expense.date : ""} className='bg-gray-700 rounded-md p-2 flex-1 min-w-0' {...register("date", { required: "Date is required", max: { value: new Date().toISOString(), message: "Date must not be in future" } })} />
            ...
            <div className='flex gap-4 items-center'>
              ...
              <div className='flex-1 flex flex-col gap-1'>
                <select
                  className='bg-gray-700 rounded-md p-3 w-full text-white'
                  // Remove defaultValue, let react-hook-form control value via setValue or defaultValues
            ...
            {/* 
              To show default values in react-hook-form, 
              you should set them in useForm({ defaultValues: { ... } }) 
              or use setValue in a useEffect when editing.
              Remove defaultValue from <select> and let react-hook-form handle it.
            */}
            <div className='flex gap-4 items-start'>
              <h1 className='text-2xl w-32 shrink-0'>Description</h1>
              <textarea rows="8" type="text" placeholder='Enter Description' className='bg-gray-700 rounded-md p-2 flex-1 min-w-0' {...register("description")} defaultValue={id ? expense.description : ""} />
            </div>
          </form>
        </div>
